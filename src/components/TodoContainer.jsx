...
export default function TodoContainer() {
  ...
  return (
    <div className='p-5  bg-[url("/images/bg2.jpg")] h-screen'>
    ...
    <div  className="rounded-md shadow-lg lg:w-1/2 xl:w-1/2 md:w-1/2 m-auto space-y-1 p-2 mt-10 border border-purple-300 font-[popins]  ">
        ...
        <input  value={task.content} onChange={handleChange} className='border-b-2 rounded-lg shadow-sm w-full p-2.5 focus:outline-none' type="text" placeholder='what would you like to add? ' name="content" id="" />
        ...
        <button  onClick={editing ? update : handleClick} className='px-7 py-1 rounded-lg shadow-md border border-purple-500 hover:text-white hover:bg-purple-400'> {editing ? 'Update' : "Add"}</button>
       {editing ?<button onClick={cancelEdit} className='px-6 py-1 rounded-lg shadow-md border border-red-500 hover:text-white ml-2 hover:bg-red-400'> Cancel</button>:null } 
        </div>
...
<svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" fill="currentColor" className={`bi bi-trash3 cursor-pointer text-red-500 ${editing?'hidden':null}`} viewBox="0 0 16 16">
